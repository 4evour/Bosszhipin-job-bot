...
def test_versions_in_sync():
    ...
    wheel_tauri = tomllib.loads(
        (WHEEL_TAURI_DIR / "Tauri.toml").read_text(encoding="utf-8")
    )
    tauri_conf = json.loads(
        (SRC_TAURI_DIR / "tauri.conf.json").read_text(encoding="utf-8")
    )
