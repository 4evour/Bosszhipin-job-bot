...
EventKind = Literal[
    ...
    "page_opened",  # 页面已打开，payload: {requested_url, actual_url, redirected}
    "page_filter",  # 页面筛选状态，payload: {group, value, status, message}
    "login_ok",  # 登录态确认
    "job_found",  # 抓到一个 JD，payload: {index, jd_preview}
    "job_matched",  # 扫描模式命中岗位，payload: {index, status, jd_preview}
    "job_skipped",  # 跳过当前岗位，payload: {index, reason, detail, score?, threshold?, matched?}
    "review_requested",  # GUI 审核模式等待用户决策，payload: {index, job, details, greeting}
    "review_cleared",  # GUI 审核卡片可清空，payload: {index, decision}
    "letter_sent",  # 招呼语处理完成，payload: {index, status, letter_len, match_explanation?, match_details?}
...
class ProgressEvent(BaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        if not self.ts_local:
            self.ts_local = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
