...
项目里所有默认落盘路径（``./chrome_profile`` / ``./logs`` / ``.env``）
都是 **CWD 相对**的：
...
env var 覆盖（``BOSS_CHROME_PROFILE`` / ``LETTER_LOG_PATH`` 等）依旧优先，
绝对路径不受 chdir 影响。
"""
...
def ensure_app_data_cwd() -> Path:
    ...
    之后所有 CWD 相对的默认路径（logs/ chrome_profile/ .env）都自动落在
    数据目录下。非 standalone 模式**不要**调它。
    """
