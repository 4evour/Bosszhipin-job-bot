...
- ⛔ 体积仍大：嵌入式 Python 和浏览器自动化依赖都会进入 bundle
...
0.2 已删除只服务旧 AI 流程的重依赖，standalone 体积应明显小于旧版本。仍需关注：

- 嵌入式 Python 解释器本身体积。
- PyTauri / Tauri 运行时资源。
- nodriver 和浏览器自动化相关依赖。
- `standalone` extra 不应重新引入旧 AI/RAG 依赖。

## 后果

- `src-tauri/**` 变更由 `.github/workflows/rust.yml` 的 `cargo check` 兜底
- 发版 checklist 多一步：四处版本号同步（测试会拦）
- standalone 的用户数据（.env / chrome_profile / logs）落在
